...
Java_com_xiaopeng_myrxjavaretorofitmvpjni_util_JNIUtils_get3DESKey(
        JNIEnv *env,
        jclass type,
        jobject context) {
    ...
    {
        return (env)->NewStringUTF("ABCCBJR$mobile%0813[!SECRET@]&");
    } else {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_xiaopeng_myrxjavaretorofitmvpjni_util_JNIUtils_get3DESVi(
        JNIEnv *env,
        jclass type,
        jobject context) {
    ...
    if (strcmp(signStrng, RELEASE_SIGN) == 0)//签名一致  返回合法的 api key，否则返回错误
    {
        return (env)->NewStringUTF("20170213");
    } else {
        return (env)->NewStringUTF("");
    }
}
